...
export default function CyberBackground() {
  ...
  const calculateNodes = useCallback((width: number, height: number) => {
    const gridSize = Math.max(
      MIN_GRID_SIZE,
      Math.min(MAX_GRID_SIZE, Math.floor(Math.sqrt(width * height) / 20))
    );
  ...
  const nodes = useMemo(
    () => calculateNodes(dimensions.width, dimensions.height),
    [dimensions, calculateNodes]
  );
    ...
    const animate = () => {
      ...
      if (mousePosition.current) {
        ...
        if (!smoothMousePositionRef.current) {
          ...
          const newX =
            ...
            (targetX - smoothMousePositionRef.current.x) * 0.1;
          const newY =
            smoothMousePositionRef.current.y +
            (targetY - smoothMousePositionRef.current.y) * 0.1;
          if (
            Math.abs(newX - smoothMousePositionRef.current.x) > 0.1 ||
            Math.abs(newY - smoothMousePositionRef.current.y) > 0.1
          ) {
      ...
      nodes.forEach((node, index) => {
        ...
        const distance = smoothMousePositionRef.current
          ? Math.sqrt(
              (node.x - smoothMousePositionRef.current.x) ** 2 +
                (node.y - smoothMousePositionRef.current.y) ** 2
            )
          : Infinity;
